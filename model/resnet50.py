...
class resnet50_Head(nn.Module):
    def __init__(self, num_classes=1, channel=64, bn_momentum=0.1):
        ...
        self.head_512 = nn.Sequential(
            ...
            nn.ConvTranspose2d(1, 1, kernel_size=4, stride=2, padding=1),  # 128->256
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(1, 1, kernel_size=4, stride=2, padding=1),  # 256->512
        )

        self.head_512_adv = nn.Sequential(
            nn.Conv2d(num_classes, 1, kernel_size=1, stride=1, padding=0),
            nn.ConvTranspose2d(1, 1, kernel_size=4, stride=2, padding=1),  # 128->256
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(1, 1, kernel_size=4, stride=2, padding=1),  # 256->512
    ...
    def forward(self, x):
        ...
        heat_map_512 = self.head_512(shared_feat).sigmoid_()
        heat_map_512_adv = self.head_512_adv(shared_feat).sigmoid_()
        return  heat_map_512, heat_map_512_adv
